  ...
  return (
    ...
    >
      {options.map(({ value, label, icon: Icon }) => (
        <button
          key={value}
          className={`flex cursor-pointer items-center gap-1.5 rounded-md border-0 px-3 py-1.5 text-[13px] transition-colors duration-150 hover:text-ink ${state.viewport === value ? "bg-raised text-primary shadow-flat" : "bg-transparent text-ink-muted"}`}
